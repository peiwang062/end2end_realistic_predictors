...
PIXEL_DEPTH = 255
NUM_LABELS = 100
VALIDATION_SIZE = 1500  # Size of the validation set.
...
LAMBDA_POOL = np.arange(10.0, 100.0, 1.0, dtype=np.float32)
EPOCH_STAGE1 = 30
...
def main(_):
    def train(train_data, train_labels, validation_data, validation_labels, test_data, test_labels, CUR_LAMBDA):
        ...
        all_easiness_tr_save = np.zeros([50000, 1])
        ...
        with tf.Graph().as_default() as graph:
            ...
            fc1_weights_m = tf.get_variable("fc1_w_m", [8 * 8 * 64, 512], trainable=True)
            ...
            loss_m_2 = loss_main_network2(predicted_labels, predicted_hardness_te, train_labels_node, tf.nn.sigmoid(100.0 * (predicted_error - 0.5)))
            ...
            loss_m_1 = loss_m_1 + 5e-4 * regularizers_m
            ...
            loss_m_2 = loss_m_2 + 5e-4 * regularizers_m
            ...
            predicted_error_te = tf.subtract(tf.constant(1.0, dtype=tf.float32, shape=(BATCH_SIZE, 1)),
                                         tf.nn.sigmoid(100.0 * (predicted_error_te - 0.5)))
            predicted_error_te = tf.nn.relu(predicted_error_te)
        ...
        with tf.Session(graph=graph, config=config) as sess:
            ...
            for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
                ...
                if step % EVAL_FREQUENCY == 0:
                    ...
                    if total_test_error < accuracy_temp and total_test_error > 0.1:
                        ...
                        all_easiness_tr = np.zeros([50000, 1])
            ...
            all_hardness_te = np.zeros([10000, 1])
            for step_te in xrange(int(test_size) // BATCH_SIZE):
                offset_te = (step_te * BATCH_SIZE) % (test_size)
                batch_data_te = test_data[offset_te:(offset_te + BATCH_SIZE), ...]
                batch_labels_te = test_labels[offset_te:(offset_te + BATCH_SIZE)]
                feed_dict_te = {train_data_node: batch_data_te,
                                train_labels_node: batch_labels_te, phase_train: False}
                easiness = sess.run(predicted_error_te, feed_dict=feed_dict_te)
                hardness = sess.run(predicted_hardness_te, feed_dict=feed_dict_te)
                final_all_easiness_te[offset_te:(offset_te + BATCH_SIZE), 0] = np.squeeze(easiness)
                all_hardness_te[offset_te:(offset_te + BATCH_SIZE), 0] = np.squeeze(hardness)

            final_all_easiness_tr = np.zeros([50000, 1])
            all_hardness_tr = np.zeros([50000, 1])
            for step_tr in xrange(int(train_size) // BATCH_SIZE):
                offset_tr = (step_tr * BATCH_SIZE) % (train_size)
                batch_data_tr = train_data[offset_tr:(offset_tr + BATCH_SIZE), ...]
                batch_labels_tr = train_labels[offset_tr:(offset_tr + BATCH_SIZE)]
                feed_dict_tr = {train_data_node: batch_data_tr,
                                train_labels_node: batch_labels_tr, phase_train: False}
                easiness = sess.run(predicted_error_te, feed_dict=feed_dict_tr)
                hardness = sess.run(predicted_hardness_te, feed_dict=feed_dict_tr)
                final_all_easiness_tr[offset_tr:(offset_tr + BATCH_SIZE), 0] = np.squeeze(easiness)
                all_hardness_tr[offset_tr:(offset_tr + BATCH_SIZE), 0] = np.squeeze(hardness)

        sess.close()
        return accuracy_temp, all_easiness_te_save, final_all_easiness_te, all_hardness_te, total_test_error, final_all_easiness_tr, all_easiness_tr_save
    ...
    if FLAGS.self_test:
        ...
        new_train_labels = np.zeros(50000, dtype=np.int32)
        for i in range(50000):
            new_train_labels[i] = train_labels[i,0]
        train_labels = new_train_labels
        new_test_labels = np.zeros(10000, dtype=np.int32)
        for i in range(10000):
            new_test_labels[i] = test_labels[i,0]
    ...
    test_size = test_labels.shape[0]
    total_acc = []
    all_tr_easiness = np.zeros([LAMBDA_POOL.shape[0], 5, 50000, 1])
    all_te_easiness = np.zeros([LAMBDA_POOL.shape[0], 5, 10000, 1])
    final_all_te_easiness = np.zeros([LAMBDA_POOL.shape[0], 5, 10000, 1])
    final_all_tr_easiness = np.zeros([LAMBDA_POOL.shape[0], 5, 50000, 1])
    all_te_hardness = np.zeros([LAMBDA_POOL.shape[0], 5, 10000, 1])
    ...
    for i in range(LAMBDA_POOL.shape[0]):
        ...
        for j in range(5):
            accuracy_tmp, all_easiness_te, final_all_easiness_te, all_hardness_te, final_te_acc, final_all_easiness_tr, all_easiness_tr = train(train_data, train_labels, validation_data,
            ...
            final_all_tr_easiness[i, j, :, :] = final_all_easiness_tr
            all_te_hardness[i, j, :, :] = all_hardness_te
            ...
            sio.savemat('final_all_tr_easiness_save_different_lambda.mat', {'final_all_tr_easiness': final_all_tr_easiness})
            sio.savemat('all_te_hardness_save_different_lambda.mat', {'all_te_hardness': all_te_hardness})
