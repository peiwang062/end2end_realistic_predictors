...
LAMBDA_POOL = np.arange(1.0, 50.0, 1.0, dtype=np.float32)
EPOCH_STAGE1 = 30
...
def main(_):
    def train(train_data, train_labels, validation_data, validation_labels, test_data, test_labels, CUR_LAMBDA):
        ...
        with tf.Session(graph=graph, config=config) as sess:
            ...
            final_all_easiness_tr = np.zeros([60000, 1])
            all_hardness_tr = np.zeros([60000, 1])
            for step_tr in xrange(int(train_size) // BATCH_SIZE):
                offset_tr = (step_tr * BATCH_SIZE) % (train_size)
                batch_data_tr = train_data[offset_tr:(offset_tr + BATCH_SIZE), ...]
                batch_labels_tr = train_labels[offset_tr:(offset_tr + BATCH_SIZE)]
    ...
    test_size = test_labels.shape[0]
    total_acc = []
    all_tr_easiness = np.zeros([LAMBDA_POOL.shape[0], 5, train_size, 1])
    all_te_easiness = np.zeros([LAMBDA_POOL.shape[0], 5, test_size, 1])
    final_all_te_easiness = np.zeros([LAMBDA_POOL.shape[0], 5, test_size, 1])
    final_all_tr_easiness = np.zeros([LAMBDA_POOL.shape[0], 5, train_size, 1])
    all_te_hardness = np.zeros([LAMBDA_POOL.shape[0], 5, test_size, 1])
